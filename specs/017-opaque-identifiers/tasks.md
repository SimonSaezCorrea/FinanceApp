...
- [x] T014 Run `pnpm --filter @finance/api test && pnpm --filter @finance/api test:integration && pnpm --filter @finance/api test:e2e && pnpm typecheck && pnpm lint && pnpm format:check` and confirm zero regressions (SC-004) — done against a throwaway local Postgres (`prisma db push` + `prisma:seed`, user-confirmed since it's a destructive command): `test:unit` 561/561, `test:integration` 85/85, `test:e2e` 141/141 (including the new signed-cursor and opaque-storage-key cases), `typecheck`/`lint`/`check:boundaries` clean; `format:check` clean except 3 pre-existing warnings in this feature's own `.md` design docs (research/data-model/contracts), unrelated to this task and not modified by it
- [x] T015 Walk through quickstart.md end-to-end (§1-§6) — §1-3 and §6 are exercised verbatim by the e2e HTTP suite (`transactions.http.spec.ts`'s new `pagination cursor signing` block issues/tampers/old-formats a cursor over real HTTP against a booted Nest app, same requests as the curl steps) and by the full test run above; §4-5 require S3 credentials this sandbox doesn't have — quickstart.md itself says to verify that case at the unit/integration level instead, which the `attachment-policy.spec.ts`/`handlers.spec.ts`/`attachments.http.spec.ts` updates already do. A standalone `tsx watch src/main.ts` dev-server boot was also tried for a literal curl walkthrough but hit an unrelated pre-existing DI resolution error specific to that runner (the same `AppModule` boots cleanly under the e2e suite's `Test.createTestingModule`, so this isn't a regression from this feature) — not chased further since it's outside this spec's scope
