...
import { CreateInstallmentPlanHandler } from "../../../../../../src/domains/installments/application/commands/create-installment-plan.handler";
import { CreateInstallmentPlanCommand } from "../../../../../../src/domains/installments/application/commands/create-installment-plan.command";
  ...
  it("generates the schedule and persists it via the repository", async () => {
    const create = vi.fn().mockImplementation(async (userId: string, plan) =>
      InstallmentPlan.fromPersistence({
        ...
        notes: plan.notes,
        payments: plan.payments.map((p: { sequence: number; dueDate: Date; amount: string }, i: number) => ({
          id: `pay${i}`,
          sequence: p.sequence,
          dueDate: p.dueDate,
          amount: p.amount,
    ...
    expect(create).toHaveBeenCalledWith("u1", expect.objectContaining({ title: "Laptop", installmentCount: 3 }));
  });
});
