...
import { PrismaBankAccountRepository } from "../../../../../src/domains/accounts/infrastructure/prisma-bank-account.repository";
import { PrismaTransactionRepository } from "../../../../../src/domains/transactions/infrastructure/prisma-transaction.repository";
...
describe("PrismaTransactionRepository (integration)", () => {
  ...
  const accountRepo = new PrismaBankAccountRepository(prisma);
  ...
  it("findAccount returns the account's rule-relevant context", async () => {
    const ctx = await txRepo.findAccount(userId, creditAccountId);
    expect(ctx?.type).toBe("CREDIT_LINE");
    expect(ctx?.creditLimit).toBe("1000000");
  });

  it("findOrCreateOpenStatement creates one, then reuses it", async () => {
    const first = await txRepo.findOrCreateOpenStatement(creditAccountId);
    const second = await txRepo.findOrCreateOpenStatement(creditAccountId);
    expect(second.id).toBe(first.id);
  });

  it("saveNew creates the transaction and atomically increments creditUsed", async () => {
    const stmt = await txRepo.findOrCreateOpenStatement(creditAccountId);
