...
describe("CloseSessionHandler (integration)", () => {
  ...
  it("closing a session stamps closedAt (row survives) and its refresh token can no longer be used", async () => {
    ...
    const closeHandler = new CloseSessionHandler({ publish: () => {} } as never, sessionRepo);
    await closeHandler.execute(new CloseSessionCommand(userId, tokens.sessionId));
  ...
  it("throws SESSION_NOT_FOUND for a session that isn't the caller's own", async () => {
    ...
    await expect(
      closeHandler.execute(new CloseSessionCommand("someone-else", tokens.sessionId)),
