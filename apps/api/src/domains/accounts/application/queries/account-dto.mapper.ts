...
export async function accountsToDtos(
  ...
  const txs = await accountRepo.txWindow(userId, ids, since);
  ...
  const cardsInfo = rows.flatMap((r) => r.cards.map((c) => ({ id: c.id, billingCycleDay: r.billingCycleDay })));
  const cardSums: CardSums = new Map();
  for (const s of await accountRepo.cardSums(userId, cardsInfo)) {
