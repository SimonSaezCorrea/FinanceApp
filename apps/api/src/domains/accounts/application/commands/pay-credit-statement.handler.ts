...
import { AccountNotFoundError, InvalidPaymentSourceError, NothingToPayError, StatementNotFoundError } from "../../domain/errors";
import {
...
> {
  ...
  // Cross-aggregate persistence (FR-020, contracts/layer-contracts.md): the
  // payment Transaction is a raw Prisma write here (the `transactions` domain
  // is out of scope for this migration) — created inside the SAME transaction
  // as the two aggregate saves so all three commit or roll back together.
