...
export function CardDetailPanel({
  ...
  // approximation.
  const filters = { bankAccountId: account.id, cardId: card.id };
  const { data: summary } = useTransactionsSummary(filters);
  const { data: recent } = useTransactions(
    inline ? undefined : { ...filters, limit: RECENT_LIMIT },
  );
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {movementsHint ? (
          <p className="text-xs leading-snug text-muted-foreground">
            {t("transactions.count", { count: summary?.total ?? 0 })}. {movementsHint}
          </p>
        ) : null}
        {recentRows}
      </div>
