  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange} title={t("accounts.new")}>
      <div className="grid gap-6 md:grid-cols-2">
        <div className="flex flex-col gap-3">
          ...
          <Field label={t("accounts.form.institution")} htmlFor="m-inst">
            <Input id="m-inst" value={institution} onChange={(e) => setInstitution(e.target.value)} />
          </Field>
          <Field label={t("accounts.form.status")} htmlFor="m-status">
            <Select id="m-status" value={status}
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label={t("accounts.form.currency")} htmlFor="m-cur">
              <Input id="m-cur" value={currency} maxLength={3}
                onChange={(e) => setCurrency(e.target.value.toUpperCase())} />
            </Field>
            <Field label={t("accounts.form.initialBalance")} htmlFor="m-bal">
              <Input id="m-bal" inputMode="decimal" value={initialBalance}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => setAddingCard((v) => !v)}>
            ...
            {cards.map((c, i) => (
              <div key={i} className="flex items-center justify-between rounded-md border px-3 py-2 text-sm">
                ...
                <Button type="button" variant="ghost" size="sm"
                  onClick={() => setCards((p) => p.filter((_, j) => j !== i))}>✕</Button>
              </div>
            ))}
            {addingCard ? (
              <CardForm
      ...
      <div className="mt-6 flex justify-end gap-2">
        ...
        <Button onClick={submit} disabled={create.isPending || !name}>{t("accounts.new")}</Button>
      </div>
    </Dialog>
  );
}
