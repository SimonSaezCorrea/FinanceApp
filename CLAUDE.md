...
- **`apps/api`** — **NestJS 11** (Express 5), the **sole owner of the database** (Prisma 7 / PostgreSQL, connected via the `@prisma/adapter-pg` driver adapter — Prisma 7 no longer accepts a `datasource.url` in `schema.prisma`; the connection string lives in `apps/api/prisma.config.ts` (CLI) and is passed to `PrismaService`'s constructor via `ConfigService` (app runtime); `prisma/seed.ts` builds its own adapter the same way). **Table-first: one DB table = one folder under `src/domains/<table>/`** (kebab-case, matching the table's `@@map`), each split into the four DDD layers `domain/`, `application/`, `infrastructure/`, `presentation/` (specs/009 + the one-table-one-domain amendment below; the old flat `*.service.ts`/`*.repository.ts` skeleton is gone, and tests live in `apps/api/test/{unit,integration,e2e}/` mirroring `src/`). The 22 table-domains: bank-account, billing-settings, credit-statement, card-account, card-limit, transaction, wallet-item-dashboard, installment-plan, installment-payment, debt, savings-goal, savings-entry, recurring-expense, user, country, currency, country-currency, country-identifier-type, financial-institution, institution-account-type, transaction-attachment, idempotency-record — plus `import` and `health`, the only folders that own no table. (The `investment` and `etf-price-cache` table-domains existed 2026-08-15 through 2026-09-07 and were removed — see the "Investment tracking removed" amendment below; `AccountType.INVESTMENT` itself is untouched, still a valid `bank-account` type, just not offered when creating/editing an account for now.) Cross-cutting in `src/infra/` (`prisma` single client, `auth` `JwtAuthGuard` + `@CurrentUser`, `http` error filter + `ZodValidationPipe`, `config`, `cron` scheduled automations via `@nestjs/schedule` — each `*.cron.ts` is a thin trigger dispatching a `scope: "system"` command into its domain, e.g. `billing-generation.cron.ts` → `credit-statement`'s `GenerateAllDueStatementsCommand`). Global prefix `/api/v1`. **DB table names are kebab-case via `@@map`** (e.g. `bank-account`, `card-account`, `wallet-item-dashboard`); Prisma model names stay PascalCase. Auth is **pure JWT email+password** — the NextAuth `Account`/`Session`/`VerificationToken` tables were removed (no OAuth adapter in the API).
  ...
  - **Investment tracking removed (2026-09-07):** the standalone `investment` table-domain (ETF/
    ...
    link to a `BankAccount`) and its never-implemented companion `etf-price-cache` (Alpha Vantage
    quote cache scaffold — table + ports, no real fetch logic) are both **deleted**: schema models,
    `apps/api/src/domains/investment/` and `.../etf-price-cache/`, their module registrations in
    `app.module.ts`, `packages/contracts/src/investments/`, the seeded rows, and every unit/
    integration/e2e test for them. `ALPHA_VANTAGE_API_KEY` dropped from `.env.example` — it was never
    actually read by any config/service, only reserved for this feature. Reason: not used and not
    planned for a long while, so the code is cut rather than carried as dead weight — reviving it
    later means restoring from git history, not maintaining an unused domain in the meantime.
    **Unaffected:** `AccountType.INVESTMENT` remains a valid `BankAccount` type in the schema/contract
    (a bank account "de inversión" is still representable) — the web `AccountTypeToggle` just
    disables picking it in create/edit (`accounts.form.investmentTypeLocked`, "Próximamente"), and the
    frontend's own `/investments` dashboard route/nav item/domain folder were removed in the same
...
- **Constitution v2.1.0 (2026-09-03) — §VI domain count fixed (24), Principle VII gets its reference
  ...
  "21 table-domains use the four layers" — neither was ever accurate; replaced with a qualitative
  description that won't drift the next time a domain is added). See this document's `idempotency-record`
  domain bullet above for the mechanism itself.
...
servidor recuerda cada intento con su resultado: repetir un intento devuelve la misma respuesta sin
efecto adicional, mientras que dos operaciones distintas que se parecen —dos cafés iguales— siguen
entrando las dos, porque la identidad la da la clave y **nunca** el contenido. Ésa es la tensión que
descarta la forma (b) del principio VII: cualquier llave natural sobre monto/fecha/cuenta/descripción
bloquearía el segundo café. Tabla nueva **`idempotency-record`** (dominio propio, principio VI) cuyo
`@@unique([userId, key])` **es el candado**, no una validación: dos peticiones simultáneas hacen el
mismo INSERT y Postgres deja pasar una. Protocolo de dos fases — reservar `IN_FLIGHT` en transacción
propia, después **efecto + `COMPLETED` en UNA sola transacción**. Esa atomicidad es toda la seguridad
del diseño: implica que `IN_FLIGHT` ⟹ el efecto no se confirmó, y de ahí que tomar uno vencido (60 s)
sea seguro por argumento y no por optimismo. Completar DESPUÉS de confirmar el efecto parece
equivalente y no lo es — una caída entre medio deja el efecto aplicado y el intento marcado como no
aplicado, y el reintento duplica. Consecuencia estructural: el **handler** pasa a ser dueño de la
transacción donde hoy no lo es (`saveNew` y `saveTransferPair` la abren adentro del adapter; los 7
comandos de `debt` no abren ninguna), con `*WithTx` nuevos — mismo patrón que
`pay-credit-statement.handler.ts` ya usa con cinco puertos. Se descartó resolverlo con un interceptor
global (como el de logging) porque vive fuera de la transacción del efecto. Se agrega el camino de
corrección de un **aporte a meta de ahorro** (`GET/PATCH/DELETE /savings/entries/:id`; el puerto sólo
tenía `list` y `create`, y el agregado se declaraba inmutable) y las guardas que le faltan a `Debt`
(`settle()` re-estampa `settledAt` en cada llamada; `undoPayment()` des-liquida de más). **Hallazgos
de la auditoría que cambiaron el alcance**: (1) `apiClient.apiFetch` **ya reintenta escrituras hoy** —
ante un 401 renueva la sesión y reenvía la petición original, un camino de duplicación que nadie había
identificado, y que queda protegido gratis porque el header viaja dentro del `init` que se repite; (2)
la **importación quedó fuera de alcance** al verificar que `POST /import/transactions` no tiene ningún
cliente (la ruta web es un placeholder, cero llamadas en `apps/web`) y no aplica deltas de saldo ni
cupo; (3) el **progreso de una meta de ahorro no existe** —ni columna, ni suma, ni contrato, ni UI—
así que FR-013 quedó escrito como regla para cifras futuras y construirlo es spec aparte; (4) el guard
del pago de facturación **no es concurrente** (lee `paidAt` antes de abrir la transacción), así que se
protege igual pese a tener máquina de estados terminal. Límite conocido y aceptado: recargar la página
en medio de un envío pierde la clave y un reenvío duplicará — evitarlo exige persistir borradores.
Sin dependencias nuevas, sin migración (`db:push`). Implementa el **principio VII** por su forma (c) y
cierra la entrada 4 de la deuda de conformidad de `docs/PENDING.md`. **Estado: 015 implementado**
(T001-T078; contrato, dominio `idempotency-record`, refactor `*WithTx` en `transaction`/`debt`/
`savings-entry`, diez endpoints protegidos, cron de purga, corrección de aporte a meta de ahorro
[`GET/PATCH/DELETE /savings/entries/:id`], docs y constitución en v2.1.0).)

Prior plan: specs/014-installment-credit-billing/plan.md
...
Earlier plan: specs/013-installments-redesign/plan.md
